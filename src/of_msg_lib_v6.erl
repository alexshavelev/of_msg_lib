...
%%%
%%% @end
%%% Created : 13. Авг. 2018 10:43
%%%-------------------------------------------------------------------
-module(of_msg_lib_v6).
-author("alex_shavelev").
...
-export([create_error/2,
  ...
  flow_modify/3,
  flow_delete/2,
  flow_monitor_request_add/4,
  flow_monitor_request_delete/4,
  flow_monitor_request_modify/4,
  group_add/3,
  group_modify/3,
  group_delete/2,
  set_port_up/3,
  ...
  set_table_mod_eviction/2,
  set_table_mod_vacancy_events/2,
  get_description/0,
...
flow_monitor_request_add(Flags,Matches,MonitorId,Opts) ->
  #ofp_flow_monitor_request{
    ...
    out_port = get_opt(out_port, Opts, any),
    out_group = get_opt(out_group, Opts, any),
    monitor_flags = get_opt(monitor_flags,Opts,[]),
...
%% up/down
set_port_up(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [],
    mask = [port_down],
    properties = [ mk_port_mod_properties(Property) || Property <- Properties ]
  }.

set_port_down(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [port_down],
...
set_port_recv(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [],
...
set_port_no_recv(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [no_recv],
...
set_port_fwd(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [],
...
set_port_no_fwd(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [no_fwd],
...
set_port_packet_in(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [],
...
set_port_no_packet_in(Addr, PortNo, Properties) ->
  #ofp_port_mod{
    port_no = PortNo,
    hw_addr = Addr,
    config = [no_packet_in],
...
%% ofp_table_mod
set_table_mod_eviction(Properties,Opts) ->
  #ofp_table_mod{
    table_id = get_opt(table_id,Opts,all),
    config = [eviction],
...
get_queue_desc(Flags,PortNo,QueueId) ->
  #ofp_queue_desc_request{flags = Flags,
    port_no = PortNo,
    queue_id = QueueId
  }.
...
%% ofp_meter_config_request
%% Get meter configuration fo rone or all meters
get_meter_configuration(MeterId,Flags) ->
  #ofp_meter_config_request{
    flags = Flags,
    meter_id = MeterId
  }.
...
requestforward(Msg) ->
  #ofp_requestforward{ request = Msg }.

bundle_ctrl_msg(BundleId,CtrlType,Flags,Properties) ->
  #ofp_bundle_ctrl_msg{
    bundle_id = BundleId,
    type = CtrlType,
    flags = Flags,
    properties = [ mk_bundle_props(P) || P <- Properties ]
  }.

bundle_add_msg(BundleId,Flag,Msg,Properties) ->
  #ofp_bundle_add_msg{
    bundle_id = BundleId,
    flags = Flag,
    message = Msg,
...
decode(#ofp_table_desc_reply{ flags = Flags, tables = Tables }) ->
  {table_desc_reply,[{flags,Flags},
    {tables,[dec_table_desc(D) || D <- Tables]}
  ]};

decode(#ofp_queue_desc_reply{ flags = Flags, queues = Queues}) ->
  {queue_desc,[{flags,Flags},
    {queues,[ dec_queue_desc(Q) || Q<- Queues]}]};

decode(#ofp_flow_monitor_reply{ flags = Flags, updates = Updates }) ->
  {flow_monitor_reply,[{flags,Flags},
    {updates,[ dec_flow_update(U) || U <- Updates ]}]};
...
decode(#ofp_role_status{ role = Role,
  reason = Reason,
  generation_id = GenerationId,
  ...
  {role_status,[{role,Role},
    {reason,Reason},
    {generation_id,GenerationId},
    {properties,[ dec_role_prop(P) || P <- Properties ]}]};

decode(#ofp_table_status{reason = Reason, table = Table}) ->
  {table_status,[{reason,Reason},
    {table,Table}]};

decode(#ofp_requestforward{ request = Request }) ->
  {requestforward,[{request,Request}]}.
...
dec_port_stats_properties(#ofp_port_stats_prop_ethernet{ rx_frame_err = FrameErr,
  ...
  [{rx_frame_err , FrameErr},
    {rx_over_err , OverErr},
    {rx_crc_err , CRCErr},
    {collisions , Collisions}];
dec_port_stats_properties(#ofp_port_stats_prop_optical{ flags = Flags,
  ...
  [{flags, Flags},
    ...
    {bias_current, BiasCurrent},
    {temperature, Temperature}];
dec_port_stats_properties(#ofp_port_stats_prop_experimenter{experimenter = Experimenter,
  exp_type = ExpType,
  data = Data}) ->
  [{experimenter, Experimenter},
    {exp_type, ExpType},
...
dec_port_desc_property(#ofp_port_desc_prop_optical{
  ...
  [{supported,Supported},
    ...
    {tx_pwr_min,TxPwrMin},
    {tx_pwr_max,TxPwrMax}];
dec_port_desc_property(#ofp_port_desc_prop_experimenter{
  experimenter = Experimenter,
  exp_type = ExpType,
  data = Data }) ->
  [{experimenter,Experimenter},
...
dec_table_desc(#ofp_table_desc{ table_id = TableId,
  config = Config,
  properties = Properties }) ->
  [{table_id,TableId},
    {config,Config},
    {properties,[ dec_table_mod_property(P) || P <- Properties ]}].
...
dec_table_mod_property(#ofp_table_mod_prop_vacancy{vacancy_down = VacancyDown,
  ...
  [{vacancy_down,VacancyDown},
    {vacancy_up,VacancyUp},
    {vacancy,Vacancy}];
dec_table_mod_property(#ofp_table_mod_prop_experimenter{experimenter = Experimenter,
  exp_type = ExpType,
  data = Data }) ->
  [{experimenter, Experimenter},
    {exp_type, ExpType},
...
dec_queue_desc(#ofp_queue_desc{ port_no = PortNo,
  ...
  [{port_no,PortNo},
    {queue_id,QueueId},
    {properties,[ dec_queue_desc_property(P) || P <- Properties ]}].

dec_queue_desc_property(#ofp_queue_desc_prop_min_rate{ rate = Rate }) ->
  [{rate,Rate}];
dec_queue_desc_property(#ofp_queue_desc_prop_max_rate{ rate = Rate }) ->
  [{rate,Rate}];
dec_queue_desc_property(#ofp_queue_desc_prop_experimenter{ experimenter = Experimenter,
  exp_type = ExpType,
  data = Data }) ->
  [{experimenter,Experimenter},
    {exp_type,ExpType},
...
dec_flow_update(#ofp_flow_update_full{event = Event,
  table_id = TableId,
  reason = Reason,
  ...
  [{event, Event},
    ...
    {match, Match},
    {instructions, Instructions} ];
dec_flow_update(#ofp_flow_update_abbrev{event = Event, xid = Xid}) ->
  [{event,Event},{xid,Xid}];
dec_flow_update(#ofp_flow_update_paused{event = Event}) ->
  [{event,Event}].

dec_role_prop(#ofp_role_prop_experimenter{experimenter = Experimenter,
  exp_type = ExpType,
  data = Data }) ->
  [{role_prop,[{experimenter = Experimenter},
    {exp_type = ExpType},
    {data = Data}]}].
